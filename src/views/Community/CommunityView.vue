      ...
      <div class="basis-full border p-3">
        <ul class="flex flex-wrap gap-y-2">
          <li v-for="(e, index) in WeekRecipeData" :key="index" class="w-full flex flex-wrap">
            <router-link :to="{ name: 'communityDetail', query: { docId: WeekRecipeId[index] } }"
              @click="$store.commit('CommunityDetail', WeekRecipeId[index])" class="basis-full flex flex-wrap group">
              <div class="w-full sm:w-1/2 h-[150px] relative">
                <span
                  class="absolute bottom-3 right-3 bg-point px-2 py-0.5 rounded-xl text-xs text-white flex gap-x-1 z-10">
      ...
      <div class="basis-full border p-3">
        <ul class="flex flex-wrap gap-y-2">
          <li v-for="(e, index) in NewRecipeData" :key="index" class="w-full flex flex-wrap">
            <router-link :to="{ name: 'communityDetail', query: { docId: NewRecipeId[index] } }"
              @click="$store.commit('CommunityDetail', NewRecipeId[index])" class="basis-full flex flex-wrap group">
              <div class="w-full sm:w-1/2 h-[150px] relative">
                <span
                  class="absolute bottom-3 right-3 bg-point px-2 py-0.5 rounded-xl text-xs text-white flex gap-x-1 z-10">
                  <img :src="require(`@/assets/img/heart.png`)" alt="좋아요" class="mt-0.5 w-3 h-3"> {{ e.liked }}</span>
